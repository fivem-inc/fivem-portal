...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ user, onSubmissionComplete, expenses, setExpenses, profileName: parentProfileName, pendingTemplates, onTemplateApplied }) => {
  ...
  const validateDraft = useCallback(() => {
    ...
    if (!effectiveWorkplace?.trim()) missing.push('行き先');
    ...
    if (missing.length > 0) {
      ...
      const fieldMap: Record<string, string> = {
        ...
        '利用日': 'start_date', '開始日': 'start_date', '終了日': 'end_date', '行き先': 'workplace'
      };
  ...
  const handleValidateAndConfirm = () => {
    ...
    for (const expense of expensesToSubmit) {
      ...
      if (!effectiveWorkplace?.trim()) { setFormError('行き先を入力してください。'); return; }
    }
  ...
  return (
    <div>
      ...
      <form>
        ...
        {/* 複製・よく使う経路からの適用バナー */}
        {templateSource !== null && templateQueue.length === 0 && (
          <div style={{ background: '#fff3cd', border: '1px solid #ffc107', borderRadius: 6, padding: '8px 12px', marginBottom: 8, fontSize: 13, color: '#856404', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>📋</span>
            <span>{templateSource === 'route' ? 'よく使う経路を適用中' : '複製を適用中'}（利用日を入力して「追加」してください）</span>
          </div>
        )}
            ...
            <div style={{ background: isDarkMode ? '#2c3e50' : '#fff', border: '2px solid #0d6efd', borderRadius: 8, padding: 16, marginBottom: 8, boxShadow: isDarkMode ? 'none' : '0 2px 8px rgba(0,0,0,0.06)' }}>
              ...
              {(() => {
                ...
                return (
                  <div style={{ marginBottom: 8, position: 'relative' }}>
                    ...
                    {showTransportPicker && (
                      <>
                      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 199 }} onClick={() => setShowTransportPicker(false)} />
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                {draftExpense.workplace === 'その他' && draftExpense.type !== 'other' && (
                  <input type="text" placeholder="行き先を入力" value={draftExpense.workplace_other || ''} onChange={(e) => { setDraftExpense(prev => ({ ...prev, workplace_other: e.target.value })); clearHL('workplace'); }} className="expense-input form-input-full" style={{ marginTop: 4, ...hl('workplace') }} />
                )}
              </div>
