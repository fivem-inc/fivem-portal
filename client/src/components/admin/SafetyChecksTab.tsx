...
const SafetyChecksTab: React.FC = () => {
  ...
  const doResend = async (checkId: string) => {
    ...
    setBusy(false);
    if (error || data?.error) { setErrorMsg('⚠️ 再送できませんでした: ' + (data?.error || error?.message)); return; }
  ...
  const submitProxy = async () => {
    ...
    setBusy(false);
    if (error) { setErrorMsg('⚠️ 記録できませんでした: ' + error.message); return; }
  ...
  const save = async () => {
    ...
    if (editingId) {
      ...
      setBusy(false);
      if (error) { setErrorMsg('⚠️ 保存できませんでした: ' + error.message); return; }
      ...
      const { error } = await supabase.from('safety_check_templates')
        ...
        .select('id');
      setBusy(false);
      if (error) { setErrorMsg('⚠️ 追加できませんでした: ' + error.message); return; }
  ...
  const toggleActive = async (t: Template) => {
    ...
    if (error) { setErrorMsg('⚠️ 変更できませんでした'); return; }
  ...
  const doDelete = async (id: string) => {
    ...
    if (error) { setErrorMsg('⚠️ 削除できませんでした: ' + error.message); return; }
  ...
  const doCloseOrCancel = async (checkId: string, kind: 'close' | 'cancel') => {
    ...
    if (error) { setErrorMsg(`⚠️ ${kind === 'close' ? '終了' : '取消'}できませんでした: ` + error.message); return; }
