    ...
    <div style={{
      ...
      <div style={{
        ...
        >
          ...
          {canTripReport && isPub('trip_report') && (
          ...
          {/* 場所の予約。毎日開くものなので連絡板の前に置く。
              🚨 2026-08-31 にマネージャー以上へ絞った（ユーザー確定）。
                 フロア責任者は**含めない**（案A）。「マネージャー以上」に
                 フロア責任者を入れるかは毎回割れるので、役職ごとのONで持つ。
                 出し分けは管理画面の「役職・機能権限」で変えられる */}
          {canRoomBooking && isPub('room_booking') && (
...
const Dashboard: React.FC = () => {
  ...
  const encAnswerModal = encAnsweringDay ? (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#343a40', borderRadius: 16, padding: 24, width: '100%', maxWidth: 420, boxSizing: 'border-box' }}>
        ...
        {encAnswerChoice === 4 && (
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, fontSize: 13, color: '#fff', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => { setEncAnsweringDay(null); setEncAnswerChoice(null); setEncAnswerNote(''); setEncAnswerWorking(false); setEncAnswerError(''); }}
          ...
          <button disabled={!encAnswerChoice || (encAnswerChoice === 4 && !encAnswerNote.trim()) || encAnswerSubmitting}
            onClick={async () => {
              ...
              setEncAnswerSubmitting(false);
              setEncAnsweringDay(null); setEncAnswerChoice(null); setEncAnswerNote(''); setEncAnswerWorking(false); setEncAnswerError('');
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', position: 'relative', padding: '70px 16px 0', boxSizing: 'border-box' as const, width: '100%' }}>
      ...
      {/* 交通費申請フォーム。
          ここはホーム（/）なのでページごとリダイレクトはできない。
          管理画面の公開設定・役職トグルがOFFの人にはフォームだけ出さない */}
      {(isAdmin || canExpense) && isFeaturePublished('expense', featurePublishState, isAdmin, roleTitle) && (
      <ExpenseForm
        user={user}
...
const TripReportPage: React.FC = () => {
  const { user, isAdmin, isApprover, profileName, roleTitle, canLeave, canShiftReport, canCalendar, canPurchaseRequest, canOvertime, canExpense, canTripReport, canBoard, canRoomBooking, canFaq, canFaqNav, canTripReportHistory, handleLogout, loading } = useAuth();
  const featurePublishState = useFeaturePublished();
  if (!user || loading) return <div style={{ padding: 40, textAlign: 'center' }}>読み込んでいます...</div>;
  // 🚨 ナビのボタンを隠すだけでなく、ルート側にもガードを入れる
  //    （隠しても URL を直接開けば入れてしまう。連絡板で抜けていた事故と同型）
  if (!isFeaturePublished('trip_report', featurePublishState, isAdmin, roleTitle)) return <Navigate to="/" />;
...
const LeaveRequestPage: React.FC = () => {
  const { user, isAdmin, isApprover, profileName, roleTitle, canLeave, canShiftReport, canCalendar, canPurchaseRequest, canOvertime, canExpense, canTripReport, canBoard, canRoomBooking, canFaq, canFaqNav, leaveRequestEnabled, handleLogout } = useAuth();
...
const LeaveApprovalsPage: React.FC = () => {
  ...
  // 管理画面の「休暇承認」トグルを実際に効かせる（これまでどこからも読まれていなかった）
  if (!isFeaturePublished('leave_approvals', featurePublishState, isAdmin, roleTitle)) return <Navigate to="/" />;
...
const TeamCalendarPage: React.FC = () => {
  const { user, isAdmin, isApprover, profileName, roleTitle, canLeave, canShiftReport, canCalendar, canPurchaseRequest, canOvertime, canExpense, canTripReport, canBoard, canRoomBooking, canFaq, canFaqNav, canLeaveShiftAdjust, canAttendanceInput, shiftAdjust, handleLogout, loading } = useAuth();
  if (!user || loading) return <div style={{ padding: 40, textAlign: 'center' }}>読み込んでいます...</div>;
  ...
  return (
    <div style={{ padding: '70px 16px 0' }}>
      <NavBar isAdmin={isAdmin} onLogout={handleLogout} email={user.email || ''} profileName={profileName} canLeave={canLeave} canApprove={isApprover} canShiftReport={canShiftReport} canCalendar={canCalendar} canPurchaseRequest={canPurchaseRequest} canOvertime={canOvertime} canExpense={canExpense} canTripReport={canTripReport} canBoard={canBoard} canRoomBooking={canRoomBooking} canFaq={canFaq} canFaqNav={canFaqNav} roleTitle={roleTitle} userId={user.id} />
      <Suspense fallback={<PageLoader />}>
        <CalendarPage user={user} roleTitle={roleTitle} isAdmin={isAdmin} isApprover={isApprover} canShiftAdjustPerm={canLeaveShiftAdjust} canAttendanceInput={canAttendanceInput} shiftAdjust={shiftAdjust} />
      </Suspense>
    </div>
  );
};
...
const BoardPageWrapper: React.FC = () => {
  ...
  // 管理画面の「連絡板」の役職トグルを実際に効かせる（これまで読まれていなかった）
...
const OvertimePageWrapper: React.FC = () => {
  const { user, isAdmin, isApprover, profileName, roleTitle, canLeave, canShiftReport, canCalendar, canPurchaseRequest, canOvertime, canExpense, canTripReport, canBoard, canRoomBooking, canFaq, canFaqNav, canOvertimeSummary, canShiftPatternDirectory, canOvertimeMemo, canOvertimeGrid, handleLogout, loading } = useAuth();
  const featurePublishState = useFeaturePublished();
  if (!user || loading) return <div style={{ padding: 40, textAlign: 'center' }}>読み込んでいます...</div>;
  ...
  return (
    <div style={{ padding: '70px 0 0' }}>
      <NavBar isAdmin={isAdmin} onLogout={handleLogout} email={user.email || ''} profileName={profileName} canLeave={canLeave} canApprove={isApprover} canShiftReport={canShiftReport} canCalendar={canCalendar} canPurchaseRequest={canPurchaseRequest} canOvertime={canOvertime} canExpense={canExpense} canTripReport={canTripReport} canBoard={canBoard} canRoomBooking={canRoomBooking} canFaq={canFaq} canFaqNav={canFaqNav} roleTitle={roleTitle} userId={user.id} />
      <Suspense fallback={<PageLoader />}>
        <OvertimePage user={user} profileName={profileName} roleTitle={roleTitle} isAdmin={isAdmin} canSummaryPerm={canOvertimeSummary} canShiftDirectoryPerm={canShiftPatternDirectory} canMemoPerm={canOvertimeMemo} canGridPerm={canOvertimeGrid} />
      </Suspense>
    </div>
  );
};
...
const RoomBookingPageWrapper: React.FC = () => {
  const { user, isAdmin, isApprover, profileName, roleTitle, employmentType, canLeave, canShiftReport, canCalendar, canPurchaseRequest, canOvertime, canExpense, canTripReport, canBoard, canRoomBooking, canFaq, canFaqNav, handleLogout, loading } = useAuth();
  const featurePublishState = useFeaturePublished();
  if (!user || loading) return <div style={{ padding: 40, textAlign: 'center' }}>読み込んでいます...</div>;
  // 🚨 ボタンを隠すだけでなく、URLを直接打たれても入れないようにする
  //    （他のページと同じ守り方に合わせる）
  if (!isFeaturePublished('room_booking', featurePublishState, isAdmin, roleTitle)) return <Navigate to="/" />;
  if (!isAdmin && !canRoomBooking) return <Navigate to="/" />;
  return (
    <>
      <NavBar isAdmin={isAdmin} onLogout={handleLogout} email={user.email || ''} profileName={profileName} canLeave={canLeave} canApprove={isApprover} canShiftReport={canShiftReport} canCalendar={canCalendar} canPurchaseRequest={canPurchaseRequest} canOvertime={canOvertime} canExpense={canExpense} canTripReport={canTripReport} canBoard={canBoard} canRoomBooking={canRoomBooking} canFaq={canFaq} canFaqNav={canFaqNav} roleTitle={roleTitle} userId={user.id} />
      <Suspense fallback={<PageLoader />}>
