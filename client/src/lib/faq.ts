...
// ── お客様向けFAQの「電話のご案内」（2026-09-17）──
// 番号は四条本校 総合受付（075-255-4401）。受付時間は、公開Q&A「電話番号・受付時間を知りたい」の
// 四条本校の時間（昼休みで分かれた詳しい時間）に揃える（2026-09-17 ユーザー確定）。
// 🚨 日曜は出さない。日曜の受付は会員様向けで、新規のお問い合わせは月〜土（2026-09-17 ユーザー確定）。
//    この案内は「答えが見つからなかった人」向けで、入会を考えている方も見るため。
// 🚨 同じ日に「フッターの時間」→「お問い合わせページの画像の時間（10:00〜20:00）」→「Q&Aの時間」と
//    2回変わった。ホームページの画像とフッターはまだ別の時間のままなので、揃えるならホームページ側を直す。
// 🚨 番号・呼び名・鍵・初期値はここ1か所。ウィジェット・エラー時の画面・管理画面の見本が同じものを使う
//    （以前は3か所に番号が直書きされていた）。受付時間は管理画面 → FAQ から管理者が直せる。
export const FAQ_CONTACT_PHONE = '075-255-4401';
export const FAQ_CONTACT_PHONE_LABEL = '四条本校 総合受付';
export const FAQ_PHONE_HOURS_KEY = 'faq_contact_phone_hours';
/** 設定が読めないとき・空のときに出す時間（公開Q&A「電話番号・受付時間を知りたい」の四条本校・2026-09-17 時点） */
export const FAQ_PHONE_HOURS_DEFAULT = ['月〜金 9:30〜12:15／13:15〜20:00（水は19:45まで）', '土 9:30〜12:40／13:40〜18:00'];
