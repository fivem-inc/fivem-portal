...
const HEADER_HINTS: Record<CustomerField, string[]> = {
  ...
  // 🚨 「名前_姓」「ふりがな_名」のように **後ろに姓/名が付く見出し** を先に当てる。
  //    これが無いと「名前_姓」が『名前を含む＝氏名の列』と判定され、
  //    名と名カナが取り込まれない（2026-08-31 スコラプラスの実データで発覚）。
  //    norm() が「_」を落とすので、ここでは「_」抜きで書く。
  last_name:     ['名前姓', '氏名姓', '生徒姓', '姓', '名字', '苗字', 'lastname', 'せい'],
  first_name:    ['名前名', '氏名名', '生徒名前名', '名', 'firstname', 'めい'],
  last_kana:     ['ふりがな姓', 'フリガナ姓', 'かな姓', 'カナ姓', 'よみ姓', 'ヨミ姓',
                  '姓カナ', 'セイカナ', 'せいかな', 'lastkana'],
  first_kana:    ['ふりがな名', 'フリガナ名', 'かな名', 'カナ名', 'よみ名', 'ヨミ名',
                  '名カナ', 'メイカナ', 'めいかな', 'firstkana'],
