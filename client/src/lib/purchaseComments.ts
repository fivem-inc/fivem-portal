...
export async function postPurchaseComment(params: {
  ...
  try {
    ...
    // 🚨 共有には「何をすればよいか」を必ず添える（2026-09-07 実機の事故対応）。
    //    「◯◯が共有されました」だけだと、受け取った人が読んでも次の動作が分からず、
    //    実際に2週間だれも確認を押さないまま止まっていた（「見たけど何をするのか分からない」）。
    //    🚨 画面のボタンの文字（✓ 確認した）とそろえること。
    //       「確認しました」と書くと、その文字は画面のどこにも無いので探して見つからない。
    // 🚨 質問側は触らない。赤い「回答待ち」という専用の出口がすでにあり、
    //    ここに催促を足すと本文が押し出されて質問の中身が読めなくなる
    //    （subject は管理画面から編集できる値でもあるため、二重表示にもなりうる）。
    const sub = params.kind === 'share' && files.length > 0
      ? `${files[0].label}${files.length > 1 ? ` 他${files.length - 1}件` : ''}が共有されました${body ? `：${body.slice(0, 30)}` : ''}。開いて［✓ 確認した］を押してください`
