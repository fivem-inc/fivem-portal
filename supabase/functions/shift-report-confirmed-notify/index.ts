...
serve(async (req) => {
  ...
  try {
    ...
    // 🚨 ここにあった「提出者の所属チームを組み立てる処理」は削除した（2026-09-11）。
    ...
    //    既定値は立場のコード（leader / manager）。申請者本人は DB 側で必ず除外される
    //    （既に別途「受理されました」通知が届くため）。
    async function resolveTargetIds(recipient: string | null): Promise<string[]> {
      ...
      if (error) { console.error('[shift-report-confirmed-notify] 宛先を解決できません', error.message); return [] }
