-- 備品購入申請 複数商品対応 ステップ1（その6）
-- 複数商品をまとめて新規申請・再申請するためのRPC。SECURITY INVOKERでRLSに従う方針は既存と同じ。
-- 新規申請時は呼び出し側で事前に生成したUUIDをp_request_idとして渡し、そのままpurchase_requests.idに使う
-- （明細のpurchase_request_idを先に確定させるため）。

CREATE OR REPLACE FUNCTION public.submit_purchase_request(
  p_request_id uuid,
  p_is_resubmit boolean,
  p_header jsonb,
  p_items jsonb
) RETURNS uuid
LANGUAGE plpgsql
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO public.purchase_requests (
      ...
      p_header->>'store_name', p_header->>'purpose', p_header->>'notes', p_header->>'amount_diff_reason',
