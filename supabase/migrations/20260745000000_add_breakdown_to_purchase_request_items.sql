-- 備品購入申請：商品ごとの「内訳・仕様」メモを追加
--
-- 【背景】「ウレタンマット Sサイズ×2・Mサイズ×1」のような内訳を書きたいが、
--   商品を分けるほどでもない・備考や見積コメントに「詳細はマネージャー回覧」と書いて逃げていた。
--   構造化（名称＋個数＋金額の行を追加していく形）は複数商品機能と完全に重複し、
--   金額の自動計算（選んだ業者の単価×数量）とも矛盾するため、
--   **金額に関係しない自由メモ**に徹する。金額が異なる内訳は商品を分ける運用
--   （フォームに「この商品をコピーして追加」を用意した）。
--
-- ⚠️ このプロジェクトの教訓に従い、列追加とRPC更新を同一マイグレーションで行う。
--   ベースは 20260743000000（デグレ修復済みの最新版）。20260741000000 は
--   reason・jsonb_typeof防御が欠けた壊れた版なので絶対にベースにしない。
...
BEGIN
  ...
  LOOP
    INSERT INTO public.purchase_request_items (
      ...
      single_vendor_reason, breakdown
    ) VALUES (
