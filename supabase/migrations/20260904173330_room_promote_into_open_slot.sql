-- ============================================================
-- 場所予約：募集中の枠にも繰り上げできるようにする（2026-09-04 ユーザー承認・案A）
--
-- きっかけ：募集中の枠にキャンセル待ちが5名並んでいるのに、繰り上げようとすると
--   「この回は募集中の枠です」と断られた。**募集枠＝まだ空いている**のだから、
--   待ちの順に入れられるのが正しい。
--
-- 🚨 2026-09-02（⑪）で当方が「募集枠への繰り上げは止める（申込で埋めるのが正しい）」と
--    決めたが、**募集枠に待ちが並ぶ運用**を想定できていなかった。ここで方針を変える。
--
-- 直し方：対象が募集中の枠なら、休講にして新しい予約を作るのではなく、
--   **既存の room_fill_open_slot で同じ行を埋める**。
--   🚨 枠を消して作り直さない。消してから作ると、その一瞬に他の人が入り込む
--      取り合いが起きる（2026-08-28 の決定）。
--   🚨 定員2以上の枠は埋めてもまだ空きが残る（still_open）。次の方も続けて繰り上げられる。
--   🚨 担当の重なりは今までどおり先に見る（繰り上げ済みの予約と二重にならないように）。
--   🚨 繰り上げの記録（room_waitlist_promotions）も予約のときと同じように残す。
--      created_booking_id は**枠そのもののID**（埋めた行が結果の予約になるため）。
--
-- ロールバック手順:
--   20260904171415 の room_promote_waitlist_at を流し直す
-- ============================================================
...
begin
  ...
  -- ★ 募集中の枠は「すでに空いている」ので、そのまま繰り上げる（2026-09-04 ユーザー承認・案A）。
   --   🚨 枠を消して作り直さない。**同じ行を埋める**（room_fill_open_slot）。
   --      消してから作ると、その一瞬に他の人が入り込む取り合いが起きる（2026-08-28 の決定）。
   --   🚨 定員2以上の枠は埋めてもまだ空きが残る（still_open）。次の方も続けて繰り上げられる。
   --   🚨 2026-09-02（⑪）で「募集枠への繰り上げは止める」としたが、**募集枠に待ちが
   --      並ぶ運用**を想定できていなかった。ここで方針を変える。
  if v_t.kind = 'open' and v_t.deleted_at is null and v_t.status = 'active' then
    ...
    select * into v_res from room_fill_open_slot(
      v_t.id, coalesce(v_w.member_no, ''), v_w.customer_label, coalesce(v_w.note, ''));
    if not coalesce(v_res.ok, false) then
      return query select false, coalesce(v_res.reason, '募集枠に入れられませんでした'), null::uuid;
      return;
    end if;
    -- 記録は予約と同じように残す（二重繰り上げの判定・一覧の「最後の繰り上げ」に使う）
    begin
      insert into room_waitlist_promotions (waitlist_id, booking_id, created_booking_id, created_by)
      values (v_w.id, v_t.id, v_t.id, auth.uid());
    exception when unique_violation then
      update room_waitlist_promotions p
         set created_booking_id = v_t.id, created_at = now(), created_by = auth.uid()
       where p.waitlist_id = v_w.id and p.booking_id = v_t.id;
    end;
    if v_w.recurrence_id is not null then
      update room_waitlist
         set last_promoted_booking_id = v_t.id, last_promoted_at = now(), updated_at = now()
       where id = p_waitlist_id;
    else
      update room_waitlist
         set status = 'promoted', promoted_booking_id = v_t.id,
             last_promoted_booking_id = v_t.id, last_promoted_at = now(), updated_at = now()
       where id = p_waitlist_id;
    end if;
    return query select true, null::text, v_t.id;
    return;
  end if;
