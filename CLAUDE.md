...
2.【要検討・レビュー指摘】承認待ちバナー（休暇・勤務変更）だけ30秒ポーリングが無く、
   ホームを開いたまま他の人が処理するとバッジは減るのにバナーが残る。
   既存の useLeavePendingCount / useShiftPendingCount に寄せれば解決（条件は完全一致を確認済み）
3.【要検討】欠勤登録の通知が30日で441件と多い（1人ずつ登録すると人数分飛ぶ）。
   まとめて登録機能を使う周知が第一手
4.【ユーザー操作】WordPressにFAQを設置（繰越）／【運用】月1回 質問ログを見てFAQを足す
5.【要対応・security】「ログイン済みなら誰でも読み書き」ポリシー是正（繰越）
6.【要判断】社内版FAQの残り1問（時間調整タブ）／【繰越】脆弱性high3件ほか
