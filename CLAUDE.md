...
### ⚠️ 未着手（次回の続き・ユーザー承認済みの方針）
- **備品の申請・精算に送信前の確認画面**（承認ルートと承認者名を出すのが要点。
  「3万円超のため全員承認になります」を送信前に見せれば金額のケタ違いにも気づける）
- **ハイライトが無い残り5ページ**：欠勤入力(15)・残業(8)・調整休の提案(7)・出張報告(6)・勤務変更(5)
- **調整休の提案に確認画面**（相手に通知が飛ぶのに確認が無い）
