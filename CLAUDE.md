...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 🚨 レビューで見つかった別問題（今回は触っていない・別PR候補）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. **交通費(expenses)の管理者判定がメールアドレス直書き**
   `auth.jwt() ->> 'email' = 'fivem.kyoto@gmail.com'`。→ ①社長は交通費を1件も見られない
   ②そのメールを変えた瞬間に全交通費が管理不能になる。profiles にも同種が2本（info@five-m.com 等）
2. **`(auth.jwt() ->> 'role') = 'admin'` の誤記が3テーブルに残存**
   leave_requests(select_admin) / shift_reports(approver_select) / attendance_exceptions。
   トップレベルの role は常に 'authenticated' なので**永久にfalse**。同じ式に role_title の OR が
...
6. useAuth の writeAuthCache が、権限取得に失敗したとき perms を `{}` で上書きしてしまう
   → 次回起動時に「ナビボタンが減った状態」が出る（2026-07-13 に直した症状の再発経路）
7. 同じ意味のタブUIが4ページにコピペされ、出張報告だけ色が違っていた（#3d9a3d → #28a745 に統一済み）。
   共通 PageTabs への切り出しは影響が大きいので次回2体レビュー付きで
...
7.【繰越】勤務変更の入力漏れ色・管理者修正モーダルの実機確認／定期リマインドの1月分／年間カレンダー入力／
