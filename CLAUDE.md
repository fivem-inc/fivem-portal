...
## ▶▶ 次セッション ここから開始（2026-08-10・機能別表示権限の表を1画面に収めた）

```
【状態】実装完了。commit/push はこれから
・DB変更なし・Edge Function変更なし
・未追跡は AGENTS.md のみ（触らない）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ きっかけ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
管理画面「⚙️役職・機能権限管理」の表（機能名＋全公開／リーダー以上／社長のみ＋役職7つ＝11列）が
横幅に収まらず、社長・管理者の列が画面外に隠れて気づけなかった（PCでも要横スクロール）。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 直したこと（FeaturePermissionsTab.tsx）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
・列幅（機能150／全公開54／リーダー以上60／社長のみ54／役職各54）＋ table-layout:fixed で
  合計696pxに詰め、コンテナの実質幅（max-width 820 − padding 24 = 796px）に収める
・🚨 **`box-sizing: border-box` を付け忘れると、padding が width の外側に上乗せされて
  想定より広がる**（772px指定のはずが実測820pxまで膨張していた）。table・th・td 全部に必要
・役職名の `whiteSpace: 'nowrap'` を外し折り返し許可（「フロア責任者」「マネージャー」対応）
・トグルボタンを36px→32pxにわずかに縮小
・PC幅（1024px・980px相当）で横スクロールなしに収まることを検証済み
  （client/public/ に一時HTMLを置いてVite経由で実測→動作確認後に削除。ログイン必須画面のため
  実アプリはこの環境から直接見れない。実測方法は自動メモリ [pc-width-verification] 参照）
・スマホ幅は従来通り横スクロール（overflowX:auto は維持。モバイルでは元々スクロールが要る）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 次にやること
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1.【最優先】commit・push（未実施）
2.【実機確認】PCで管理画面→役職・機能権限管理を開き、社長・管理者列まで
   横スクロールなしで見えるか。トグルが押せるか。スマホでは従来通り横スクロールになるか
3.【実機確認・前回分】リーダー・マネージャーからパートへ休暇申請フォームを送る/取り消すが
   引き続き動くか（受理ページ・管理画面の両方。RPC経由に変更したため要確認）
```
