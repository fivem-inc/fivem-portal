...
## 📌 いまの読み方（2026-08-31・2人で並行して開発しています）

このファイルには **「ここから開始」の節が2つ**あります。担当が分かれているためです。

- **場所予約（/rooms）を触る人** … すぐ下の「**2026-08-31 続き**」の節を読む
  （その次の「2026-08-31（本番公開）」の節も、機能の作りが書いてあるので目を通す）
- **備品購入申請・残業・FAQ・その他を触る人** … その次の
  「**2026-08-31・備品購入申請／バックアップ**」の節を読む（別の担当者の記録）

🚨 どちらも生きている引き継ぎです。**片方だけ読んで判断しないこと。**
🚨 共通ファイル（App.tsx / useAuth.ts / FeaturePermissionsTab.tsx / CLAUDE.md）は
   両者が触ります。作業前に必ず声をかけ合うこと。
...
1.【実機確認・今日の分】備品購入申請
  ...
  ＜共有ファイル＞「💬 質問する」→種類（確定見積書が既定）と添付ボタン→
    送信→上部に「📎 共有ファイル（1件）」→閉じてもボタンに件数が出るか→
    **承認した5名に通知が届くか**（2行目「確定見積書が共有されました」）→ファイルを開けるか
...
## ▶ 前セッション（2026-08-29 まとめ）

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【状態】すべて本番反映・push済み（最新 8810756 ＋このdocsコミット）。ローカル＝リモート一致
・本番DB：お客様向けFAQの本文を1件 update（migration 20260829000000・適用済み）
・Edge Function の変更なし
・未追跡は AGENTS.md のみ（触らない・コミットに含めない）
・🆕 今日から別の開発者が参加（部屋予約機能／ブランチ feature/rooms／room_ で始まる
  新テーブルのみ追加）。**同じファイルを触っている前提で、開始時の git pull を徹底する**
・⚠️ 8810756（事後報告の時刻制限）は**実機未確認**。残業ページはログインが要るため
  Claude 側では画面を開けない。次セッションで結果を確認する

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 今日やったこと（コミット4本）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
2a89e9b docs: このリポジトリが公開されていることを共通認識として明記した
e7fb91a fix: 実施していない校のコースが選べてしまう問題を直した（お客様向けFAQ）
0a997e1 docs: 今日の作業と注意事項を引き継ぎに保存した
8810756 feat: 事後報告を勤務が始まる前に出せないようにした（残業・時間管理）
（＋このdocsコミット）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ ① .env が GitHub にある件（別の開発者からの報告）→ 事実を確認して記録した
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
報告：「client/.env がGitHubにある。Privateなので外部からは見えないが、ルール上は望ましくない」
→ 🚨 **「Privateだから見えない」は誤り。実測で Public と確認した**
　・未ログインで GitHub API が 200 を返す／raw で .env の中身が誰でも読める
　・入っているのは公開前提の3つだけ（VITE_SUPABASE_URL／ANON_KEY／VAPID_PUBLIC_KEY）
　・全履歴を検索しても service_role・VAPID_PRIVATE の混入なし＝**実害はない**
→ これは事故ではなく 2026-07-02 のユーザー判断（下の「🔐 秘密情報の置き場所」に経緯あり）
→ **本当のリスクは「Privateだと思っている人が .env に秘密の鍵を1行足すこと」**。
　`docs/開発ルール.md` の冒頭に節を新設し、この作業ルールにも1項目追加した

🚨 **pre-commit フックの実挙動を検証した**（推測で書いて一度間違えたので、必ず実物を見る）
　client/.env            → **止まる**（新規でも、既存ファイルへの追記でも）
　client/.env.production → **素通り**（本番ビルドに必要なので意図的に対象外）
　※フックは**PCごと**の設定。**新しいPCには入っていない**＝相手のPCでは何も止まらない

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ ② お客様向けFAQ：実施していない校のコースが選べていた
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
きっかけ：「洛西口校ウェルネスはプライベートを実施していないので、表記はなしで結構です」
→ 画面で確かめると、**洛西口校を選んでもコース一覧に「ウェルネス体操プライベート」が出て、
　選ぶとキャンセル料の案内まで表示**されていた。同じ理由で西陣・上桂・南草津でも
　「ウェルネス体操」が選べた

【直したこと1】本文（本番DB適用済み・migration 20260829000000）
　旧「ウェルネス体操・ウェルネス体操プライベートは、四条本校・洛西口校で実施しています。」
　新「ウェルネス体操は、四条本校・洛西口校で実施しています。
　　　※ウェルネス体操プライベートは四条本校のみです。」（B案・行を分ける）

【直したこと2】コースの選択肢を校で絞った（lib/faq.ts に FAQ_COURSE_SCHOOLS を新設）
　マットレ＝四条本校／ジュニア姿勢・体幹＝四条本校・洛西口校／
　ウェルネス体操＝四条本校・洛西口校／ウェルネス体操プライベート＝四条本校／
　上級・養成＝上桂校　※こども器械体操とそのプライベートは全5校のため書かない
　実施校は既存のFAQ本文から読み取って表にして提示し、ユーザーに確認済み
　（こども器械体操プライベートだけ根拠が無く、質問して「全5校」と回答をもらった）
　ローカルで洛西口校・上桂校・四条本校を実機確認し、意図どおり絞られることを確認

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 🚨 この周辺を触るときの注意
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
・🚨 **FAQ_COURSE_SCHOOLS に推測で足さない**。書いた瞬間その校では選べなくなり、
　実際に通っている方が自分のコースを選べず**質問ごと画面から消える**
　（回答が1つも無い質問は一覧に出ない仕組みのため）。確認が取れたものだけ書く
・🚨 **「質問ごとの絞り込み」と「校の実態での絞り込み」は別物**。
　前者は禁止（共通回答に落ちるべき人が自分を選べなくなる）。今回入れたのは後者
・校が未選択のとき（校を聞かない質問・「この中にない・わからない」）は全コース出す
　＝**逃げ道は塞がない**
・「校を選び直す」は setViewer({}) で校もコースも初期化するので、
　実在しない組み合わせが残ることはない
・🚨 **山科教室の開講時は FAQ_SCHOOL_OPTIONS と FAQ_COURSE_SCHOOLS の両方に足す**
　（片方だけだと、山科を選んだ人にウェルネスが出ない／出てはいけないものが出る）
・🚨 **画面の見た目だけで「直った」と判断しない**。今回も、本文を直しても
　選択肢の導線は残っていた。**入口（選択肢）と中身（本文）は別々に確認する**

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ ③ 残業：事後報告を勤務が始まる前に出せないようにした（8810756）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
きっかけ：「事後報告はいまいつからできる？ 当日ならいつでも？」という質問
→ 調べると **当日なら時刻に関係なくいつでも出せた**（チェックは日付だけ）。
　まだ働いていない朝に「今日の残業」を事後報告として送信できる状態だった

【直したこと】OvertimePage.tsx（DB変更なし・フロントのみ）
　・当日ぶんの事後報告は、**入力した勤務の開始時刻を過ぎるまで送信できない**
　　例）9:30〜19:00 と入力 → 朝8:00 は「まだ 9:30 になっていません」で止まる
　・まだ終了時刻を過ぎていないときは、確認画面にアンバーの帯で一言そえる（**止めない**）
　　例）昼12:00 →「まだ勤務の終了時刻（19:00）を過ぎていません。
　　　　このまま事後報告をされますか？」→ 送信する で通る

【設計の判断（ユーザー確定）】
　・🚨 基準は通常シフトではなく **本人が入力した勤務時間**。
　　シフト基準にすると休日出勤（その日のシフトが無い）を判定できず、
　　早出（シフト9:30の日に8:00から働いた）が「まだ9:30前」で止まってしまう
　・調整休・欠勤（終日）と打刻ズレは**対象外**＝今までどおり朝でも出せる
　　（欠勤は当日の朝に出すのが普通のため）
　・過去日・実績報告・再提出も対象外（既に一度出したものなので）

【🚨 この周辺を触るときの注意】
　・🚨 **現在時刻は `nowMinLive()` で送信ボタンを押した瞬間に取る（state に持たない）**。
　　フォームを開いたまま置く人がいるので、描画時の時刻で判定すると
　　「開いた時は勤務前だったが、いまは勤務後」を取りこぼす
　・🚨 履歴カード側の `reportGateMin`（事前申請の実績報告ゲート＝min(シフト終了,予定終了)）
　　とは**別物**。あちらは「事前申請したものをいつ報告できるか」、
　　こちらは「新規の事後報告をいつ出せるか」。基準が違うので揃えないこと
　・深夜跨ぎは `workSegments` の endMin が +1440 されるため、
　　`minToTime` が「翌1:00」と表示する（`% 1440` しないこと）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 次にやること（優先順）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1.【実機確認・最優先】残業の事後報告の時刻制限（8810756・**まだ誰も動かしていない**）
  ・今日の日付で事後報告 → まだ来ていない時刻を入力 → 送信 →
    赤い帯で「まだ ◯:◯◯ になっていません」と止まるか
  ・いまより後に終わる時間を入れて送信 → 確認画面に**アンバーの帯**が出て、
    「送信する」でそのまま通るか
  ・勤務が終わったあとの報告は、今までどおり何も出ずに送れるか
  ・欠勤（終日）は朝でも出せるか（対象外にしてある）
2.【要送信】幹部への連絡（LINE・B案を推奨）は**文面のみ作成済み・未送信**。
  🚨 上の1を確認してから送る。文面は下の「■ 幹部への連絡（未送信）」
3.【実機確認・今日の分】お客様向けFAQ（ログイン不要で開ける）
  https://fivem-portal.vercel.app/faq-widget.html
  ・「欠席・振替」→ 洛西口校 →「ウェルネス体操プライベート」が**出ない**こと
    （Claude 側で本番の画面を操作して確認済み。念のため他の校も）
  ・上桂校 → 上級・養成が出て、マットレ・ウェルネスが出ないこと
  ・四条本校 → 上級・養成以外の6コースが出ること
  ・「ウェルネス体操はどの校で受けられますか？」の本文が2行に分かれていること
  ・「この中にない・わからない」が全校で残っていること（逃げ道を塞いでいないか）
4.【要伝達】新しく参加する開発者に **「このリポジトリは Public」** であることを伝える。
  あわせて pre-commit フックの設定を渡す（**PCごとの設定なので相手のPCには入っていない**）
  → 共有用のページは `docs/開発ルール.md`（これを渡せば足りる）
5.【実機確認・依頼済み】残業の「実績を報告する」（3ead881）
...
6.【要送信】残業の実績報告についての連絡文（B案＝報告者への返信／C案＝全体周知）は
  作成済み・**未送信**。下の「■ 連絡文（未送信）」を参照
7.【実機確認】テンキー 第2段（d0fe6b4）＝**見た目だけ未確認**。動作は部品共通なので問題ないはず
  ・勤怠カレンダーの欠勤入力（出勤/退勤の時刻・時間帯の開始/終了）
  ・勤務変更報告（通常シフト・実際の勤務）／休暇の時間調整（調整遅出・調整早退）
  ・通知の受信時間帯／調整休の提案・回答
  見るところ：**2枠が潰れていないか・折り返していないか**（ダークモードも）
8.【実機確認】Excel出力の「差分(分)」と合計／履歴の理由と詳細／受理画面の提出日時・振替元
9.【繰越】プッシュ受信時間帯／通常シフトの第2時間帯／ベル連動／勤怠カレンダーの残業表示／
  購入申請まわり／Slack通知のON設定
10.【要確認・税理士】領収書の保存年数と紙廃棄の可否
11.【ユーザー操作】WordPressにFAQ設置
　🚨 設置後にスマホの戻るボタンを必ず確認（iframe内で効くかは設置しないと分からない）
12.【10月以降】山科教室の開講時：**FAQ_SCHOOL_OPTIONS と FAQ_COURSE_SCHOOLS の両方**に追加
  → 欠席・振替（洛西口の回答）の対象に追加 → 新質問の本文に実施校とレッスンタイムを追記
13.【運用・年1回】バックアップの復元リハーサル

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 幹部への連絡（作成済み・**未送信**・LINE用／B案を推奨）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
お疲れ様です。
社内サイトを2点更新しましたのでお知らせします。

■ 残業の「事後報告」
これまで当日であれば時刻に関係なく報告でき、
勤務前の朝でも送信できる状態でした。

・勤務の開始時刻を過ぎるまでは送信できません
・まだ終了時刻を過ぎていないときは確認が出ます
　（そのまま送信することは可能です）
・判定は入力された勤務時間が基準なので、
　休日出勤・早出もそのまま報告できます
・欠勤、調整休（終日）、打刻ズレは
...
■ お客様向けFAQ
洛西口校を選んでも「ウェルネス体操プライベート」が
選択肢に出て、実施していないコースの案内が
表示される状態でした。
実施している校のコースだけが出るよう修正しています。
