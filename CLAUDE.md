...
・⚠️ `client/.env.production` が pre-commitフックの条件 `(^|/)\.env($|\.local$)` に
  一致せず**すり抜ける**（実測確認済み）。今の中身は公開前提の鍵だけで実害はないが、
  **ここに秘密情報を書かないこと**。本番ビルドに必要なファイルなのでフックを広げると
  更新できなくなるため、ルールで守る方針にした
...
0-2.【実装・小】CLAUDE.mdに「client/.env.production には公開前提の情報しか書かない」と明記
   （pre-commitフックをすり抜けるため。フックを広げると本番ビルド用の更新ができなくなる）
