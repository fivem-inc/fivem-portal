...
- **相見積もりの段階表示**：いまは見出しが「価格比較（任意）」なのに1万円以上は2社以上必須で、
  しかも「金額を直接入力する（相見積もりを使わない）」が最初から選べるため矛盾している。
  **金額は業者を選んで初めて決まる**ので順序が破綻していた。方針＝
  ①まず業者1を入力 → ②単価×数量で金額が決まる → ③1万円以上になったら業者2の欄を自動で出し
  見出しを「価格比較（2社以上が必要）」に変えて赤枠にする → ④**「1社しか選べない（理由を書く）」の
  逃げ道を用意**（取扱いが1社だけ・緊急対応で相見積もりが取れない実務があるため。理由は承認者にも見える）
