...
### 次回やること（優先順）
1. 【実機確認】🕐勤務時間変更：時間帯を入れて登録 → GCalが `｜12:45〜16:50［四条本校］`・濃いグレーになるか／
   一覧の2行目に「勤務：12:45〜16:50［四条本校］」が出るか／スマホの丸印が濃いグレーで出るか
2. 【実機確認】勤務地変更と勤務時間変更を両方チェックできないか（片方を押すともう片方が外れる）。
   2回に分けて登録しようとすると赤いメッセージでブロックされるか
3. 【実機確認】一覧の「取消」→ 2択が出るか。「取消してこの内容を利用して入力する」で
   入力シートに元の内容（対象者・時間帯・校・備考）が入った状態で開くか
4. 【実機確認・要注意】**取消のプッシュ通知「取消 1件」がChromeで警告表示にならないか**（下記の学び参照）
5. 【実機確認】管理画面（休暇申請タブ）の欠勤一覧から取消したときも通知が届くか
...
- **プッシュ通知の新しい単語は実機で確認するまで安全と言えない**。検証済みは
  「新着／本日期限／明日期限／差戻／未承認」。今回の「取消 1件」は構造は同じ（漢字の名詞＋件数）だが未検証。
  Chromeが「不正な疑いのある通知」に差し替えたら `新着 1件` に変える
