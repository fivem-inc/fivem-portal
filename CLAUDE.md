...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【状態】すべて本番反映・push済み（最新 e7fb91a ＋このdocsコミット）。ローカル＝リモート一致
...
■ 次にやること（優先順）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1.【実機確認・今日の分】お客様向けFAQ（ログイン不要で開ける）
...
2.【要伝達】新しく参加する開発者に **「このリポジトリは Public」** であることを伝える。
  あわせて pre-commit フックの設定を渡す（**PCごとの設定なので相手のPCには入っていない**）
3.【実機確認・依頼済み】残業の「実績を報告する」（3ead881）
...
4.【要送信】担当への連絡文（B案＝報告者への返信／C案＝全体周知）は作成済み・**未送信**。
  下の「■ 連絡文（未送信）」を参照
5.【実機確認】テンキー 第2段（d0fe6b4）＝**見た目だけ未確認**。動作は部品共通なので問題ないはず
  ・勤怠カレンダーの欠勤入力（出勤/退勤の時刻・時間帯の開始/終了）
  ・勤務変更報告（通常シフト・実際の勤務）／休暇の時間調整（調整遅出・調整早退）
  ・通知の受信時間帯／調整休の提案・回答
  見るところ：**2枠が潰れていないか・折り返していないか**（ダークモードも）
6.【実機確認】Excel出力の「差分(分)」と合計／履歴の理由と詳細／受理画面の提出日時・振替元
7.【繰越】プッシュ受信時間帯／通常シフトの第2時間帯／ベル連動／勤怠カレンダーの残業表示／
  購入申請まわり／Slack通知のON設定
8.【要確認・税理士】領収書の保存年数と紙廃棄の可否
9.【ユーザー操作】WordPressにFAQ設置
　🚨 設置後にスマホの戻るボタンを必ず確認（iframe内で効くかは設置しないと分からない）
10.【10月以降】山科教室の開講時：**FAQ_SCHOOL_OPTIONS と FAQ_COURSE_SCHOOLS の両方**に追加
