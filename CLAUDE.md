...
## ▶▶ 次セッション ここから開始（2026-08-18 プッシュの飛び先・文言を総点検して直した）

```
【状態】Edge Function 5本デプロイ済み（DB変更なし・クライアント変更なし）
  leave-approved-notify v3／shift-report-confirmed-notify v8／attendance-notify v9／push-dispatch v22
・FAQ39番（欠席振替パス）の中身を具体化＝DB適用済み。FAQ管理に「🔄 要更新」の絞り込みを追加
・未追跡は AGENTS.md のみ
...
・実際は「申請」ではなく「受理されました」の共有通知2件。押しても対象が見えない場所に着地していた
  ①休暇の受理FYI → /calendar（今月が開くだけ。対象は2027年2月の休暇＝画面に無い）
  ②勤務変更の受理 → /shift-report（既定の入力フォームに着地。履歴タブに切り替えないと見えない）
・🚨 原因はどちらも「ベルは reference_id を持つのにプッシュのURLに反映していない」＝別管理の弊害

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 直したもの（全部デプロイ済み・これから届く分から有効）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
| 通知 | 文面 | 飛び先 |
|---|---|---|
| 休暇の受理FYI | 新着→**受理** | /calendar?focus=<初日>&view=fyi（ベルと同じ） |
| 勤務変更の受理 | 新着→**受理** | /shift-report?tab=history&focus=<報告ID> |
| 欠勤登録 | 変更なし | /calendar?focus=<対象日>（**取消はfocus無し**＝対象行が消えているため） |
| 欠勤登録の見出し | 「欠勤・遅刻・早退」→**「勤怠」** | 休日出勤・勤務地変更・勤務時間変更も同じ入口のため |
| 定期リマインド | 変更なし | /board → **/**（ホームの専用バナーへ。8/3にバナーを作った時の直し残し） |
| 休暇マネージャー受理・残業の受理系 | 新着→**受理** | 変更なし |
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 次にやること：プッシュ→ベル連動（A案・ユーザー承認済み・未着手）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【目的】プッシュとベルを2回チェックする手間をなくす
【決定】A案＝プッシュを押したら**ベル一覧が開いて該当行が光る**（押せば既読＋移動＝いつもの動き）
　※C案（直行＋一言バー）も検討したが、まとめプッシュのとき結局ベルを開くことになるためAに決定
【対象の切り分け】
　・ベルを開く＝「知らせ・結果」だけ：受理FYI（休暇/勤務変更）・時間調整/欠勤の登録・立替精算の記録・
　　結果報告（受理/管理者修正/取消）・修正依頼の対応済み/対応不可
　・直行のまま＝安否/緊急・要対応（未承認/差戻）・ホームに専用バナーがあるもの（定期リマインド・
　　有給奨励日・残業超過・実績未報告・打刻確認・お知らせ）・連絡板・**ベル通知が無いプッシュ**（CC送信等）
【2体レビューで出た必須事項】
　🚨 push_queue の notification_id は**単一列でなく配列**（同一 user×event×reference は dedupe で
　　 2件目が積まれない＝IDが落ちる）
　🚨 URLの印を消すのは**既読処理が終わってから**（デプロイ直後の自動リロードと必ずぶつかる）
　🚨 URLの印は React Router 経由で消す（history.replaceState 直叩きだと他ページが書き戻す）
　🚨 nids は UUID 形式を検証してから使う（不正値1つで select 全体が 22P02 で落ちる）
　🚨 notification_id に外部キーを付けない（既読30日削除の cron が失敗する）
　🚨 sw.js の client.navigate は未制御 client で reject する→ openWindow へ fallback を入れる
　　 （**今も残っている既存の穴**。iPhoneで「押しても前の画面が出るだけ」になる）
　・実装前に必ず：Android/iPhone 実機で「?nids= 付きURLで着地するか」を①未起動②同じ画面③別画面で確認
　・プランと全レビュー結果は scratchpad の push_landing_plan.md

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ この周辺を触るときの注意（今回の追加分）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
・🚨 **ベル（App.tsx classifyNotif）とプッシュ（EVENT_MAP／直送関数のurl）は別管理**。
　 片方だけ直す事故が今回も3件出た。飛び先を変えるときは必ず両方見る
