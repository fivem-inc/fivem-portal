...
2.【要作業】電話番号46人分の初期登録（管理画面→ユーザー管理→緊急連絡先列。未登録は⚠️赤字）
   ※空のままだと未回答者に電話できず機能が空回りする
3.【要実装・ユーザー要望】リマインドの時間帯設定（深夜の扱い）
   方針：安否確認は夜中でも鳴らす／台風などの出勤確認・応援要請は深夜を避ける
4.【要判断】全体公開のタイミング。**現在は先行公開（全公開OFF・リーダー以上ON）**なので
   一般スタッフには見えていない。周知文とセットで、管理画面→権限管理→安否・緊急連絡の
   「全公開」をONにする
