...
```
【状態】
・C:\Users\kohei\fivem-portal（master）。最新コミット 67b4d43（push・突合・本番反映まで確認済み）
・未追跡は AGENTS.md のみ（触らない・コミットに含めない）
・DBマイグレーション11本すべて本番適用済み（今日 20260804000000 を追加）
...
・useSafetyPendingCount … 30秒ごとに控えを保存／取得失敗で0件に落とさない／
  保留中を未回答から除外／訓練(is_test)は控えに含めない
...
【次回やること（優先順）】
1.【実機確認・最優先】機内モードで一通り
   ①訓練を受け取る→機内モードON→安否ページに「📴 オフラインです」＋回答ボタンが出るか
   ②回答→「回答を端末に保存しました」／③ホームのバナーがグレーになるか（赤のままでないか）
   ④機内モードOFF→ホームにいるだけで自動送信されバナーが消えるか
   ⑤集計タブに理由が出るか ⑥発信タブは入力が消えないか
   ※控えは電波があるとき30秒ごとに更新。一度オンラインで開いてから機内モードにすること
2.【要実装】他の Edge Function の supabase-js バージョン固定。**実際に残り25本**
   （引き継ぎには27本・board系対応済みとあったが board-scheduled-send も @2 のまま＝未対応）
   `@2` → `@2.45.0`。次にどれかをデプロイすると Module not found で失敗する
3.【要判断・保留】安否確認の全体公開（連絡板の公開とセット）
4.【将来】「助けが必要」回答の即時プッシュ（urgent_notified_at 列は用意済み）
5.【将来・見送り中】Service Worker でアプリ本体をキャッシュ＝**完全圏外での起動**に対応。
