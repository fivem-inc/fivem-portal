...
## ▶▶ 次セッション ここから開始（2026-08-26・プッシュ通知の受信時間帯を自分で設定できるようにした）

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【状態】DB適用済み・Edge Function 4本デプロイ済み。クライアントは push 待ち（次の作業）
・未追跡は AGENTS.md のみ（触らない・コミットに含めない）
・DBマイグレーション1本 適用済み（begin/rollback 検証 → 適用 → 実測12項目すべて正常）
  20260826000000 push_preferences／push_deferred／notifications.push_urgent／
  push_queue.urgent／enqueueトリガー更新／push_muted_user_ids 関数
...
・クライアント（tsc/build/lint 0件確認済み・未コミット）：
  PushQuietHoursSection.tsx（新規）／AccountSettings／NotificationSettings／
  BoardPage／lib/notifications.ts
...
■ 次にやること（優先順）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1.【最優先】クライアントの commit・push（許可を得てから）→ 本番JSへの着地確認
  （「通知を受け取る時間を決める」「当日の連絡・緊急」のラベルで grep）
2.【実機確認・今回分】
  ＜設定＞アカウント設定・🔔通知設定の両方に「🕐通知の受信時間」カードが出るか
  （プッシュ許可済みの人のみ）。時刻を変えると下の説明文が変わるか。
  22:00〜7:00（日またぎ）の説明。開始=終了でエラー。保存→リロードで復元
  ...
  ＜バグ修正の確認＞受理済みの事前申請に対して「残業なしで報告する（確定）」が通るか
  （報告してくれたスタッフに再試行を依頼済みのはず）
  ＜休暇日＞自分の受理済み休暇の日に mute_on_leave ONで鳴らないか
3.【実機確認・繰越】受理を待たない実績報告／通常シフトの第2時間帯／ベル連動（8件）／
  勤怠カレンダーの残業表示／購入申請まわり／Slack通知のON設定（管理画面・ユーザー操作）
4.【要確認・税理士】領収書の保存年数と紙廃棄の可否
5.【ユーザー操作】WordPressにFAQ設置
6.【運用・年1回】バックアップの復元リハーサル
```
