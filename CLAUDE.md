...
どちらの「最新の引き継ぎ」も、この CLAUDE.md の末尾で**自動で読み込まれます**
（ファイル末尾の `@docs/...` 行。消さないこと）。
過去の記録は `docs/引き継ぎ-申請系-アーカイブ.md` にあり、こちらは自動では読まれません。
🚨 **アーカイブは非常に大きく全文を開けない**ので、必ず grep で必要な箇所だけ引くこと。
...
- 🚨 **既に出ているエラーと、本物の不具合を見分ける**
  ...
  2026-09-11 に**全40本**を通し、直せるものは直したあとの状態（これが基準）：
  **TS2304 は 0件／TS18046 も 0件**。残るは外部ライブラリの型定義のズレだけで **3本に計9件**
  （push-dispatch 1／receipt-bulk-zip 2／send-push 6）。
...
- 🚨 `supabase.rpc()` / `.from()` も同じ。**`.then(null, …)` や `.catch()` では拾えない**
  （拾えるのは通信エラーだけ。制約違反・関数が無い（PGRST202）などは `{ error }` として
  正常に解決されるので、**黙って消える**）。必ず `const { error } = await …` で受けて中身を見る。
  2026-09-05 に、既存の記録処理がこの形で失敗を握りつぶしていたのを見つけて直した
...
- 🚨 新しい関数を作ったら **anon の実行権限を確認して外す**
  ...
  → **`revoke execute on function <名前>(<引数の型>) from anon;` と明示的に書く**。
    そのうえで適用後に実測する：
    `select has_function_privilege('anon','public.<名前>(<型>)','execute');`
    🚨 2026-09-05 にまた踏んだ。`from public` だけでは `true` のままだった。
    **実際に呼んで確かめなければ気づけない**
- 🚨 **記録を貯める仕組みを新しく作るときは、掃除する仕組みも必ず一緒に作る**
  （毎晩の cron で古い行を消す＋1日の上限。匿名から書ける入口なら上限は必須）。
  2026-08-20 に cron の記録が **130,817件・115MB** まで膨らみ、DB全体136MBの87%を占めていた。
  🚨 **無料枠500MBの上限に達すると、データベースが読み取り専用になり、
     申請が1件も保存できなくなる**（＝補助機能が基幹業務を止める）。
  いまの使用量の確認：`select pg_size_pretty(pg_database_size(current_database()));`
- 🚨 **鍵の値をチャット・コミットメッセージ・コード内コメントに書かない／受け取らない**

### 詳しいルールの置き場所（毎回ここを見る）

CLAUDE.md に収まらない恒久ルールは `docs/引き継ぎ-申請系-アーカイブ.md` の中にあります。
**場所予約の担当者も、新しい画面を作るときは必ず参照してください**（見た目が割れます）。
🚨 このファイルは大きすぎて全文を開けません。下の語で **grep** してください。
...
| 秘密情報の pre-commit チェック | 入っている | 入っている（2026-09-01 実測。以前の「入っていない」は誤り） |
