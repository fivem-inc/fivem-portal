...
5.【繰越】8/6分：全員に一度だけ手で更新してもらう周知（文面は作成済み・未送信）
6.【繰越】8/6分：定期リマインドの1月分を作る（作らないと1月は1通も飛ばない）
7.【繰越】年間カレンダーの入力／休館日が各画面に出るか／日付ピッカーの「‹ ›」／見積書の拡大
8.【要設定】管理画面→通知設定→「admin:setup_alert」の Slack をONにする（既定OFF）
9.【要依頼】社労士確認（前から繰越）
10.【後で協議】プッシュONの人にはメールを送らない案／残業の受理済み一覧の実績報告の位置
