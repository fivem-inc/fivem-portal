...
- **未確認（実機テスト予定）**：リーダー・マネージャーが受理ページから差し戻した際、申請者へサイト内通知・メールが実際に届くか
- 同一パターンの点検候補（今回は未調査・次回以降の課題）：
  - 勤務変更申請（shift report）の受理ページからの承認・差し戻しでも、同様に申請者への通知処理が漏れていないか要確認
  - 休暇申請の「社長の最終受理（admin_approved→approved）」ステップでは、そもそも申請者への通知コード自体が存在しない（`handleApprove`内の分岐が`step2_pending`と`manager_approved`のみ）。仕様として意図的か、対応が必要か要確認
